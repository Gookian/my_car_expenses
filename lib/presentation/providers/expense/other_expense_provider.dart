...
class OtherExpenseProvider extends StatelessWidget {
  final int carId;

  const OtherExpenseProvider({
    super.key,
    required this.carId
  });
  ...
  Widget build(BuildContext context) {
    return BlocProvider<BaseExpenseCubit>(
      create: (context) => GetIt.instance<BaseExpenseCubit>(param1: CategoryType.otherExpense)..loadExpense(carId),
      child: BaseExpenseScreen(
        title: 'Другое',
        emptyDescription: 'Для данного автомобиля\n пока что нет затрат для категории другое',
        buttonTitle: 'Добавить затраты для категории другое',
        svgIconAsset: SvgIcons.other,
        category: CategoryType.otherExpense,
      ),
    );
  }
}
