...
class ParkingExpenseProvider extends StatelessWidget {
  final int carId;

  const ParkingExpenseProvider({
    super.key,
    required this.carId
  });
  ...
  Widget build(BuildContext context) {
    return BlocProvider<BaseExpenseCubit>(
      create: (context) => GetIt.instance<BaseExpenseCubit>(param1: CategoryType.parkingExpense)..loadExpense(carId),
      child: BaseExpenseScreen(
        title: 'Парковка',
        emptyDescription: 'Для данного автомобиля\n пока что нет затрат на парковку',
        buttonTitle: 'Добавить затраты на парковку',
        svgIconAsset: SvgIcons.parking,
        category: CategoryType.parkingExpense,
      ),
    );
  }
}
