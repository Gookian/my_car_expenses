...
class FuelExpenseProvider extends StatelessWidget {
  final int carId;

  const FuelExpenseProvider({
    super.key,
    required this.carId
  });
  ...
  Widget build(BuildContext context) {
    return BlocProvider<BaseExpenseCubit>(
      create: (context) => GetIt.instance<BaseExpenseCubit>(param1: CategoryType.fuelExpense)..loadExpense(carId),
      child: BaseExpenseScreen(
        title: 'Заправка',
        emptyDescription: 'Для данного автомобиля\n пока что нет затрат на заправку',
        buttonTitle: 'Добавить затраты на заправку',
        svgIconAsset: SvgIcons.gasStation,
        category: CategoryType.fuelExpense,
      ),
    );
  }
}
